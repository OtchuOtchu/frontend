...
export default function Weather() {
    ...
    const getWeatherBackground = () => {
        const weatherCode = weatherData[selectedDate]?.weather; // WeatherStore에서 가져온 날씨 코드
        const matchingImage = weatherImages.find(image => image.weather === weatherCode); // 일치하는 이미지 찾기
        return matchingImage ? `url(${matchingImage.image})` : 'bg-gray-500'; // 이미지가 없으면 기본 배경색 반환
    };
